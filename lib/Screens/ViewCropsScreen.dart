...
class _ViewCropsScreenState extends State<ViewCropsScreen> {
  ...
  Future viewCrops() async {
    ...
    final addCropRequest = http.post(
      ...
      body: jsonEncode({"farmerId": userData["userId"].toString()}),
    );

    final response =
        await Future.any([addCropRequest, Future.delayed(timeoutDuration)]);
    try {
      if (response != null) {
        Map<String, dynamic> resp = jsonDecode(response.body);

        if (response.statusCode == 200 && resp["status"] == "success") {
          setState(() {
            cropData = resp["data"];
          });
        }
      ...
      body: Stack(
        children: [
          Padding(
              ...
              child: Consumer<CropUpdateNotifier>(
                                                ...
                                                builder: (context) =>
                                                    UpdateCropScreen(
                                                      ...
                                                      type: cropData[index]
                                                              ['type']
                                                          .toString(),
                                                      quantity: cropData[index]
                                                              ['quantity']
                                                          .toString(),
                                                      farmerId: cropData[index]
                                                              ['farmerId']
                                                          .toString(),
                                                      price: cropData[index]
                                                              ['price']
                                                          .toString(),
                                                      cropId: cropData[index]
                                                              ['id']
                                                          .toString(),
                                                    )));
                                      },
